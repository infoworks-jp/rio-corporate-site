(() => {
  ...
  const records={
    construction:{kicker:'COMPANY LICENSE / 会社許可・登録',title:'建設業許可通知書 — 全3枚',page:'certificates/construction-permit.html'},
  ...
  <div class="credential-group"><div class="credential-kicker">COMPANY LICENSE / 会社許可・登録</div><article class="credential-item"><h3>建設業許可</h3><p>北海道知事許可 石第21656号</p><div class="permit-summary"><p class="permit-label">許可業種（11業種）</p><ul class="permit-industries"><li>土木工事業</li><li>大工工事業</li><li>とび・土工工事業</li><li>石工事業</li><li>タイル・れんが・ブロック工事業</li><li>鋼構造物工事業</li><li>鉄筋工事業</li><li>舗装工事業</li><li>しゅんせつ工事業</li><li>水道施設工事業</li><li>解体工事業</li></ul></div><a class="credential-view permit-view" data-record="construction" href="certificates/construction-permit.html">建設業許可通知書 3枚を見る ↗</a></article><article class="credential-item"><h3>産業廃棄物収集運搬業許可</h3><a class="credential-view" href="assets/certificates/original/industrial-waste-transport-permit.png" target="_blank" rel="noopener noreferrer">産業廃棄物収集運搬業許可証を開く ↗</a></article></div>
  ...
  const modal=document.createElement('div');modal.className='credential-modal';modal.setAttribute('aria-hidden','true');modal.setAttribute('role','dialog');modal.setAttribute('aria-modal','true');modal.setAttribute('aria-labelledby','credential-title');modal.innerHTML='<button class="credential-close" aria-label="閉じる">×</button><div class="credential-card"><div class="credential-meta"><small></small><h3 id="credential-title"></h3></div><iframe class="credential-pdf" title="原本証書"></iframe><div class="credential-fallback"><span>PDFを表示できない場合は </span><a href="assets/certificates/display/construction-permit.pdf" target="_blank" rel="noopener">原本PDFを開く ↗</a></div></div>';document.body.appendChild(modal);
  ...
  document.addEventListener('click',e=>{const b=e.target.closest('[data-record]');if(!b)return;const r=records[b.dataset.record];if(!r)return;e.preventDefault();modal.querySelector('small').textContent=r.kicker;modal.querySelector('h3').textContent=r.title;frame.src=r.page?r.page+'?v=20260924-three&embedded=1':r.pdf+'?v=20260809-single#page=1&view=Fit';frame.title=r.title;fallback.querySelector('span').textContent=r.page?'':'PDFを表示できない場合は ';fallbackLink.textContent=r.page?'3枚を別画面で開く ↗':'原本PDFを開く ↗';fallbackLink.href=r.page||r.pdf;modal.classList.add('is-open');modal.setAttribute('aria-hidden','false');document.body.classList.add('modal-open')});
