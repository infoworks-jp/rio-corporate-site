(() => {
  ...
  const modal=document.createElement('div');modal.className='credential-modal';modal.setAttribute('aria-hidden','true');modal.setAttribute('role','dialog');modal.setAttribute('aria-modal','true');modal.setAttribute('aria-labelledby','credential-title');modal.innerHTML='<button class="credential-close" aria-label="閉じる">×</button><div class="credential-card"><div class="credential-meta"><small></small><h3 id="credential-title"></h3></div><iframe class="credential-pdf" title="原本証書"></iframe><div class="credential-fallback">PDFを表示できない場合は <a target="_blank" rel="noopener">原本PDFを開く ↗</a></div></div>';document.body.appendChild(modal);
